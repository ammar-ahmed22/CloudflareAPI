...
import type {  csvValue, Row } from "../../@types/helpers/csv.d.ts"
import {
  assertEquals,
  assertNotEquals,
...
Deno.test("CSV", async (t) => {
  ...
  await t.step(
    "Read .csv file, omit column and cast values",
    async () => {
      await data.fromFile(
  ...
  await t.step(
    "Checking if size of CSV is correct",
    () => assertEquals(data.size(), 4992),
  );

  await t.step("Writing updated csv to file", async () => {
    await data.toCSV(
      path.resolve(__dirname, "../test-data/test.csv"),
    );
  ...
  await t.step("Getting row from value", () => {
    const row = data.getRowFromValue(
      "ourCustomSKU",
      "ETATLIBSOMEONEM-100B",
    );
    assertNotEquals(row, null);
  });

  await t.step("Adding column", () => {
    const withTax = data.addColumns("withTax", (row: Row) => {
      let taxPrice = 0;
      if (typeof row.competitor_price === "number") {
        taxPrice = parseFloat(
          (row.competitor_price * 1.13).toFixed(2),
        );
      }
