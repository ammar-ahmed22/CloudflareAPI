...
import type {
  CloudflareLogs,
  CloudflareParams,
} from "../../@types/api/cloudflare.d.ts";
...
export class Cloudflare {
  ...
  private createURLFromEndpoint = (endpoint: string): URL => {
    if (endpoint[0] !== "/") {
      const error = new Error(
        "string endpoint must start with '/'",
      );
      throw error.message;
    }
  ...
  public get = async (
    ...
    try {
      const fetchURL: URL = typeof endpoint === "string"
  ...
  public logs = async (
    ...
    if (range.length > 2) {
      ...
      for (let i = 0; i < range.length - 1; i++) {
        const logsURL = this.createURLWithSearchParams(
          baseURL,
          {
            start: Math.floor(range[i].valueOf() / 1000),
            end: Math.floor(
              range[i + 1].valueOf() / 1000,
            ),
            fields: fields.join(","),
            timestamps: "unixnano",
          },
