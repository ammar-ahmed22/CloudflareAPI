...
Deno.test("Datetime helper", async (t) => {
    ...
    await t.step("Creating date in the future", () => {
        const year = Datetime.createDate(d.getDate(), "future", { years: 1 });
        const day = Datetime.createDate(d.getDate(), "future", { days: 1 });
        const hour = Datetime.createDate(d.getDate(), "future", { hours: 1 });
        const minute = Datetime.createDate(d.getDate(), "future", { minutes: 1 });
        const second = Datetime.createDate(d.getDate(), "future", { seconds: 1 });
    ...
    await t.step("Creating date range", () => {
        const end = new Datetime({ year: 2022, month: 10, day: 26, hour: 17 });
        const start = Datetime.createDate(end.getDate(), "past", { days: 1 });

        // Creating range with dates every hour from 1 day ago to 5pm on October 26th
        const range = Datetime.DateRange(start, end.getDate(), 60 * 60);
